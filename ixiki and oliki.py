...
def game_close(basic_place, x_or_o, users_cords): #"закрытый" код игры, использовал для того, что бы вносить изменения, не видимые игроку
        ...
        for j in range(len(basic_place[i])):
            if basic_place[i][j] in x_symbol or basic_place[i][j] in o_symbol: #описание победы игроков по вертикали
                ...
                if basic_place[i][j] in x_symbol:
                    ...
                    if count_o_line == 3:
                        return oliki()
            if basic_place[j + 1][i - 1] in x_symbol or basic_place[j + 1][i - 1] in o_symbol: #описание победы игроков по горизонтали
                if basic_place[j + 1][i - 1] in x_symbol:
                    ...
                    if count_o_column == 3:
                        return oliki()
            if i - 1 == j and (basic_place[i][j] in x_symbol or basic_place[i][j] in o_symbol): #описание победы игроков по главной диагонали
                if basic_place[i][j] in x_symbol:
                    count_x_diagonal += 1
                    ...
                    if count_o_diagonal == 3:
                        return oliki()
            if j == 3 - i and (basic_place[i][j] in x_symbol or basic_place[i][j] in o_symbol): #описание победы игроков по обратной диагонали
                if basic_place[i][j] in x_symbol:
...
while True:                 #сам игровой процесс состоящий из всех функции выше, так же ряд проверок на "дурака"
    ...
    if user_change_x_or_o:
        if len(some_list) % 2 == 1 and some_list[-1] in ['x', 'х']:
            ...
            if game_close(start_game, user_change_x_or_o, user_change_cords): #проверка кооридант, в случае, если координата занята Иксиком или Оликом выведет конец игры
